 ...
 */
const MultiSelectedOption: VFC<MultiSelectedOptionProps> = ({
  option,
  fieldProps,
  onRemove,
}) => {
...
export const MultiSelect: VFC<MultiSelectProps> = ({
  ...
  useEffect(() => {
    ...
    setDropLabel(
      avail.length == 0
        ? "All selected"
        : !!maxOptions && sel.length == maxOptions
          ? "Max selected"
          : label,
    );
    onChange(sel);
  ...
  return (
    <Focusable>
      ...
      <Dropdown
        rgOptions={available}
        selectedOption={dropLabel}
        onChange={onSelectedChange}
        strDefaultLabel={dropLabel}
        focusable={true}
        disabled={
          available.length == 0 ||
          (!!maxOptions && selected.length == maxOptions)
        }
      />
    </Focusable>
  );
