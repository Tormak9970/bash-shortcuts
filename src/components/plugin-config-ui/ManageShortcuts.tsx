 ...
 */
const ActionButtion: VFC<ActionButtonProps<Shortcut>> = (
  props: ActionButtonProps<Shortcut>,
) => {
        ...
        <MenuItem
          onSelected={() => {
            showModal(
              // @ts-ignore
              <EditModal
                onConfirm={(updated: Shortcut) => {
                  if (PluginController.checkIfRunning(shortcut.id))
                  ...
                  PyInterop.toast(
                    "Success",
                    `Updated shortcut ${props.entry.data?.name}.`,
                  );
                }}
                shortcut={shortcut}
              />,
            ...
            showModal(
              <ConfirmModal
                onOK={() => {
                  ...
                  PyInterop.toast(
                    "Success",
                    `Removed shortcut ${props.entry.data?.name}.`,
                  );
                }}
...
const Interactables: VFC<InteractablesProps<Shortcut>> = (
  props: InteractablesProps<Shortcut>,
) => {
  return (
    <>
...
export const ManageShortcuts: VFC = () => {
  const { setShortcuts, shortcutsList, reorderableShortcuts } =
    useShortcutsState();
  ...
  return (
    <>
      ...
      {shortcutsList.length > 0 ? (
        <>
          <ReorderableList<Shortcut>
            entries={reorderableShortcuts}
            onSave={onSave}
