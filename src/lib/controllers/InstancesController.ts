...
export class InstancesController {
  ...
  private runnerPath =
    "/home/deck/homebrew/plugins/bash-shortcuts/shortcutsRunner.sh";
  ...
  async createInstance(shortcut: Shortcut): Promise<boolean> {
    ...
    if (shortcut.isApp) {
      ...
      if (await this.shorcutsController.checkShortcutExist(shortcutName)) {
        const shortcut =
          await this.shorcutsController.getShortcut(shortcutName);
        appId = shortcut?.unAppID;
      } else {
        appId = await this.shorcutsController.addShortcut(
          shortcutName,
          this.runnerPath,
          this.startDir,
          shortcut.cmd,
      ...
      if (appId) {
        this.instances[shortcut.id] = new Instance(
          appId,
          shortcutName,
          shortcut.id,
          shortcut.isApp,
        ...
        const exeRes = await this.shorcutsController.setShortcutExe(
          appId,
          this.runnerPath,
        );
        if (!exeRes) {
        ...
        const nameRes = await this.shorcutsController.setShortcutName(
          appId,
          shortcutName,
        );
        if (!nameRes) {
        ...
        const startDirRes = await this.shorcutsController.setShortcutStartDir(
          appId,
          this.startDir,
        );
        if (!startDirRes) {
        ...
        const launchOptsRes =
          await this.shorcutsController.setShortcutLaunchOptions(
            appId,
            shortcut.cmd,
          );
        if (!launchOptsRes) {
        ...
        this.numInstances--;
        PyInterop.log(
          `Failed to start instance. Id: ${shortcut.id} Name: ${shortcut.name}`,
        );
        return false;
      }
    } else {
      PyInterop.log(
        `Shortcut is not an app. Skipping instance shortcut creation. ShortcutId: ${shortcut.id} ShortcutName: ${shortcut.name}`,
      );
      this.instances[shortcut.id] = new Instance(
        null,
        shortcutName,
      ...
      PyInterop.log(
        `Adding websocket listener for message type ${shortcut.id}`,
      );
      this.webSocketClient.on(shortcut.id, (data: any) => {
        if (data.type === "end") {
          delete this.instances[shortcut.id];
          PyInterop.log(
            `Removed non app instance for shortcut with Id: ${shortcut.id} because end was detected.`,
          );
          setTimeout(() => {
            PyInterop.log(
              `Removing websocket listener for message type ${shortcut.id}`,
            );
  ...
  async killInstance(shortcutId: string): Promise<boolean> {
    ...
    if (instance.shortcutIsApp) {
      ...
      if (success) {
        PyInterop.log(
          `Killed instance. Id: ${shortcutId} InstanceName: ${instance.steamShortcutName}`,
        );
        ...
        PyInterop.log(
          `Failed to kill instance. Could not delete shortcut. Id: ${shortcutId} InstanceName: ${instance.steamShortcutName}`,
        );
        return false;
      }
    } else {
      ...
      this.webSocketClient.on(shortcutId, (data: any) => {
        if (data.type == "end") {
          setTimeout(() => {
            PyInterop.log(
              `Removing websocket listener for message type ${shortcutId}`,
            );
  ...
  async launchInstance(
    ...
    if (instance.shortcutIsApp) {
      ...
      if (!res) {
        PyInterop.log(
          `Failed to launch instance. InstanceName: ${instance.steamShortcutName} ShortcutId: ${shortcutId}`,
        );
      } else {
        const { unregister } = this.shorcutsController.registerForShortcutExit(
          appId,
      ...
      const res = await PyInterop.runNonAppShortcut(
        shortcutId,
  ...
  async stopInstance(shortcutId: string): Promise<boolean> {
    ...
    if (instance.shortcutIsApp) {
      ...
      if (!res) {
        PyInterop.log(
          `Failed to stop instance. Could not close shortcut. Id: ${shortcutId} InstanceName: ${instance.steamShortcutName}`,
        );
        return false;
      }
